...
if (isset($_GET['orderId'])) {
    ...
    if ($status->state == "COMPLETED") {
        ...
        $statusMessage = "<h2 style='color: green;'>Payment Successful!</h2>
                          ...
                          <a  href='https://app.vskbrothers.com/'><button style='background-color:green;padding:8px;color:white;cursor:pointer'>Home Page</button></a>";
    } else {

        $statusMessage = "<h2 style='color: red;'>Payment Status: " . htmlspecialchars($status->state) . "</h2>
        <a  href='https://app.vskbrothers.com/'><button style='background-color:green;padding:8px;color:white;cursor:pointer'>Home Page</button></a>";
        
        $updateSql = "UPDATE `transaction` SET `status` = $status->state WHERE `merchant_order_id` = ?";
        $stmt = $conn->prepare($updateSql);
        $stmt->bind_param("s", $orderId);
        $stmt->execute();
        $stmt->close();

        $updateSql2 = "UPDATE `oc_order` SET `payment_status` = $status->state WHERE `merchant_order_id` = ?";
        $stmt2 = $conn->prepare($updateSql2);
        $stmt2->bind_param("s", $orderId);
        $stmt2->execute();
        $stmt2->close();
