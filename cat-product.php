...
<head>
    ...
    <link href="catalog/view/javascript/jquery/swiper/css/opencart.css" type="text/css" rel="stylesheet"
        media="screen" />
    <link href="catalog/view/javascript/jquery/swiper/css/css/owl.carousel.css" type="text/css" rel="stylesheet"
        media="screen" />
    <link href="catalog/view/javascript/jquery/swiper/css/css/owl.theme.css" type="text/css" rel="stylesheet"
        media="screen" />
...
<body>
        ...
        $.ajax({
            ...
            success: function(products) {
                ...
                for (var i = 0; i < products.length; i++) {
                    ...
                    html += '<div class="image"><a href="product.php?id=' + obj["product_id"] +
                        '" ><img width="100" height=100" src="https://vskbrothers.com/image/' + obj[
                            "image"] + '" alt="' + obj["name"] + '" title="' + obj["name"] +
                        '" class="img-responsive center-block" /></a></div>';
                    html += '<div class="caption text-center">';
                    html += '<h4><a href="product.php?id=' + obj["product_id"] + '" >' + obj["name"] +
                        '</a></h4>';
                    var price = parseFloat(obj["price"]);
                    price = price.toFixed(2);
                    html += '<p class="price">&#2352;&nbsp;' + price + '</p>';
                    html += '<div>';
                    html += "<a onclick='add_order(" + obj["product_id"] + ",\"" + obj["name"] + "\"," +
                        obj["price"] + ",\"" + obj["image"] + "\",\"" + obj["admin_margin"] +
                        "\")' class='btn btn-danger btn-large cart-button' >Add to cart<a/>";
                    html += '</div>';
                    html += '</div>';
                    html += '</div>';
                    html += '</div>';
    ...
    function load_category() {
        ...
        $.ajax({
            url: "https://vskbrothers.com/api/sup-categories.php",
            type: "POST",
            data: JSON.stringify(data),
            dataType: 'json',
            contentType: 'application/json',
            success: function(category) {
                ...
                for (var i = 0; i < category.length; i++) {
                    ...
                    html += '<div class="product-layout product-list col-xs-12">';
                    html += '<div class="product-thumb transition mobile-box-bg">';
                    html += '<div class="caption text-center">';
                    html += '<h4><a href="cat-product.php?id=' + obj["category_id"] + '">' + obj["name"] +
                        '</a></h4>';
                    html += '</div>';
                    html += '</div>';
                    html += '</div>';
