...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['by_amount']) && !empty($_GET['by_amount'])) {
		...
		$oc_order_id = isset($_GET['oc_order_id']) ? $conn->real_escape_string($_GET['oc_order_id']) : '';
//print_r($oc_order_id);die();
        ...
        // Insert transaction data into the database (including merchant_order_id)
         $sql = "INSERT INTO `transaction` (`userid`, `amount`, `date`, `name`, `phone`, `tranjactionid`, `merchant_order_id`,`oc_order_id`, `response`, `type`, `status`) 
                VALUES ('$userid', '$by_amount', '$currentDateTime', '$name', '$phone', '$merchantTransactionId', '$merchantOrderId','$oc_order_id', '', 'pending', '1')";

        if ($conn->query($sql) === TRUE) {

			$sql2 = "UPDATE oc_order SET merchant_order_id = '$merchantOrderId' WHERE ocorderid = $oc_order_id";
			$conn->query($sql2);
			...
			$sql2 = "UPDATE `oc_order` SET `payment_address_id` = '2' WHERE `oc_order`.`order_id` = $oc_order_id";
			$conn->query($sql2);
