...
class DashboardController extends BaseController
{
	...
	public function dashboard(){
		...
		//$sql = "select a.date_added,b.firstname,count(*) as prd_cnt from oc_product a,oc_user b  where a.vsk_user_id=b.user_id and a.date_added >= '$week' and a.date_added <= '$today' group by b.user_id,b.firstname	,date_added order by a.date_added desc";
		$sql = "select c.username,count(*) prd_cnt from oc_product a,oc_product_description b,oc_user c where a.product_id=b.product_id and a.vsk_user_id=c.user_id  group by c.username";
    ...
    $user_id = Auth::user()->id;
    	...
    	if($user_type_id =='1'){
	        $PendingCount = DB::table( 'oc_order' )->where( 'order_status_id', '1')->count();
		...
		$order = array();
        $sql="select * from oc_order_status where status = '1' and order_status_id !=1 order by order_status_id ";
        ...
        foreach($result as $res){
            ...
            $sql2 = "select count(*) as order_count from order_management where order_status_id = '$order_status_id' and user_id = $user_id  order by order_status_id desc limit 1";
		...
		return view("dashboard",compact('products','bill','purchase','stock','users','order','PendingCount'));
	}
}
