...
@section('content') 
                   ...
                   <div class="card-body">
			       <form action="{{url('pharmacy/add_packings')}}" method="post">
                            {{ csrf_field() }}
                   <div class="modal-header">
                   <h4 class="modal-title">Packings</h4>
                   </div>
                   <div class="modal-body">					
                   <input type="text" class="col-sm-6" name="packing_name" 
		   ...
		   @foreach($managePackings as $key=>$managePackings)
                   <tr id="arrayorder_<?php echo $managePackings->id?>">
                   <td>{{ $key + 1 }}</td>
                   <td>{{ $managePackings->packing_name }}</td>
                  @if($managePackings->status == 0)
