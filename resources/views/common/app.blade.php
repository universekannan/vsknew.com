...
@if (Auth::user()->colour == 1) light-mode
...
function delete_row(row, product_id) {
    if (confirm("Are you sure you want to delete this product?")) {
        $.ajax({
            url: "{{ url('order_delete') }}/" + product_id, // Adjust the URL if needed
            type: "DELETE",
            data: {
                _token: "{{ csrf_token() }}"
            },
...
var i = {{ !empty($orderproducts) ? count($orderproducts) : 0 }};
