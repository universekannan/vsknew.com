...
<head>
  ...
  <link rel="stylesheet" href="{!! asset('dist/css/gslsxy.css') !!}">
...
<script>
  ...
  if(amount!="")
  {
    var found = false;
    var existing_qty = 0; 
    $('.pd_id').each(function(i, obj) {
        if(product_id == obj.value){
          found = true;
          existing_qty = parseInt($(".prd_qty_"+product_id).val());
        }
    });

    var name = $('#product_id3').val();
    var item_rate = parseInt($('#rate').val());
    var item_quantity = parseInt($('#quantity').val());
    item_quantity = item_quantity + existing_qty;

    var discount_price_min_weight = parseInt($('#discount_price_min_weight').val());
    if(item_quantity >= discount_price_min_weight && discount_price_min_weight != 0){
      item_rate = parseInt($('#discount_price').val());
    } 
       
    rate = item_rate * item_quantity;
    var rate = parseFloat(rate).toFixed(2);//parseFloat($('#total').val()).toFixed(2);
    ...
    if(found){
      ...
      $('#addr'+i).html("<td class='serial_num'>"+ (i+1) +"</td>"
        +"<td><input readonly value='"+name+"'  name='item_name[]' type='text'  class=' form-control'><input readonly class='pd_id' value='"+product_id+"' name='product_id[]' type='hidden'  class='form-control'><input readonly value='"+gst_amount+"' name='gst_amount2[]' type='hidden'  class='form-control'></td>"

        +"<td><input readonly value="+item_quantity+" class='form-control prd_qty_"+product_id+"' name='item_quantity[]' type='text'></td>"

        +"<td><input readonly value="+item_rate+" class='form-control prd_rat_"+product_id+"' name='item_rate[]' type='text'></td>"+"<td><input readonly value="+rate+" class='form-control prd_tot_"+product_id+"'  name='total[]' type='text'  ><input class='form-control prd_totamt_"+product_id+"'  readonly value="+rate+" name='total_amount[]' type='hidden'></td>"
...
function packaging(obj,user_id,order_product_id){
  ...
  url = "{{url('/packaging')}}/"+user_id+'/'+order_product_id;
  console.log(user_id);
  $.ajax({
    url: url,
    type: "GET",
...
$(document).ready(function () {
    $("#cus_mobile").keyup(function () {
        ...
        if (mobile.length > 0) {
            $.ajax({
                url: "{{ url('/itemsearchmobile') }}/"+mobile,
                type: "GET",
                dataType: 'JSON',
                success: function (data) {
